...
test_exif_tags = ExifTags(capture_datetime=None, iso=100, exposure_time=1.2)
...
class TestCorrectImage:
    def test_correct_image_corrects_images_with_dark_frame_and_flat_field_and_performs_diagnostics(
        self, mocker, mock_exif_tags, mock_open_flat_field_image
    ):
        ...
        expected_corrected_image = np.array(
            [
                [
                    [0.412561, 0.412561, 0.412561],
                    [0.213768, 0.237520, 0.475041],
                    [0.412561, 0.412561, -0.187438],
                ],
                [
                    [0.337520, 0.875041, 0.483768],
                    [0.50251, 0.56251, 0.50251],
                    [0.483768, 0.875041, 0.337520],
                ],
                [
                    [0.412561, 0.412561, 0.412561],
                    [1.275041, 0.573768, 0.637520],
                    [0.412561, 0.412561, -0.187438],
                ],
            ]
        )
        ...
        np.testing.assert_allclose(
            actual_corrected_image, expected_corrected_image, atol=1e-5
        )
...
class TestSaveImageWithSuffix:
    def test_save_rgb_image_with_suffix(self, mocker):
        ...
        module.save_rgb_image_with_suffix(
            rgb_image, "original image path.jpeg", "suffix"
        )

        mock_save_as_tiff.assert_called_with(
            rgb_image, "original image pathsuffix.tiff"
        )
