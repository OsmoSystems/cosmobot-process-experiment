...
def save_rgb_image_with_suffix(rgb_image, image_filepath, filepath_suffix):
    tiff.save.as_tiff(
        rgb_image,
        file_structure.replace_extension(
            file_structure.append_suffix_to_filepath_before_extension(
                image_filepath, filepath_suffix
            ),
            ".tiff",
        ),
    )
...
def correct_image(
    ...
    dark_frame_corrected_rgb_image = dark_frame.get_metadata_and_apply_dark_frame_correction(
        original_rgb_image, original_image_filepath
    )
    dark_frame_diagnostics = dark_frame.get_dark_frame_diagnostics(
        original_rgb_image, dark_frame_corrected_rgb_image, original_image_filepath
    )

    if save_dark_frame_corrected_image:
        save_rgb_image_with_suffix(
            dark_frame_corrected_rgb_image, original_image_filepath, "_dark_adj"
        )

    flat_field_corrected_rgb_image = flat_field.load_flat_field_and_apply_correction(
        dark_frame_corrected_rgb_image, flat_field_filepath_or_none
    )
    ...
    if save_flat_field_corrected_image:
        save_rgb_image_with_suffix(
            flat_field_corrected_rgb_image, original_image_filepath, "_dark_flat_adj"
        )
