...
def mock_image_helper_functions(mocker):
    ...
    mocker.patch.object(
        module.rgb.filter, "select_channels"
    ).return_value = sentinel.channel_selected_image
    mocker.patch.object(
        module.rgb.annotate, "draw_ROIs_on_image"
    ).return_value = sentinel.roi_image
    mocker.patch.object(
        module.rgb.annotate, "draw_text_on_image"
    ).return_value = sentinel.annotated_image
...
class TestImageFilepaths(object):
    def test_get_experiment_image_filepaths(self, mocker):
        mocker.patch.object(module, "get_files_with_extension").return_value = [
            sentinel.image_file_1,
            sentinel.image_file_2,
        ]
...
class TestSummaryMediaGeneration(object):
    def test_open_filter_annotate_and_scale_image(
        self, mocker, mock_image_helper_functions
    ):
        test_array = np.zeros((10, 10, 3))
        mocker.patch.object(
            module.os.path, "basename"
        ).return_value = sentinel.mock_filename
    ...
    def test_throws_when_no_timestamp_in_filename(
        self, mocker, mock_image_helper_functions
    ):
    ...
    def test_calls_draw_text_when_timestamp_in_filename(
        self, mocker, mock_image_helper_functions
    ):
        ...
        module.rgb.annotate.draw_text_on_image.assert_called_with(  # type: ignore
            sentinel.roi_image, "2019-01-01 00:00:00"
        )
