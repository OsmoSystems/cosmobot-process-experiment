...
def _file_to_dng(raspiraw_parent_path, raw_image_path):
    ...
    raspi_raw_command = os.path.join(raspiraw_parent_path, 'raspiraw/raspi_dng_sony')
...
def _directory_to_dng(raspiraw_parent_path, raw_images_dir):
    ...
    for raw_image_path in raw_image_paths:
        _file_to_dng(raspiraw_parent_path, raw_image_path)


def to_dng(raspiraw_parent_path, raw_image_path=None, raw_images_dir=None):
    ...
    if raw_images_dir:
        return _directory_to_dng(raspiraw_parent_path, raw_images_dir)

    return _file_to_dng(raspiraw_parent_path, raw_image_path)
