...
PR103J2_temperature_vs_resistance_path = pkg_resources.resource_filename(
    "osmo_camera", "test_fixtures/PR103J2_temperature_vs_resistance.csv"
)


class TestThermistorResistanceGivenTemperature:
    def test_returns_R0_at_T0(self):
        actual = module.thermistor_resistance_given_temperature(
            temperature_c=module.T_0_CELCIUS
        )
    ...
    def test_roughly_matches_datasheet_table(self):
        ...
        calculated_resistances = datasheet_table["temperature"].apply(
            module.thermistor_resistance_given_temperature
        )
        expected_resistances = datasheet_table["resistance"]

        np.testing.assert_allclose(
            expected_resistances, calculated_resistances, rtol=0.02
        )


class TestVoltageDividerVoutGivenResistances:
    @pytest.mark.parametrize(
        "name, r2, r1, v_in, expected_v_out",
        [("divides in half", 10, 10, 10, 5), ("divides in quarter", 10, 30, 10, 2.5)],
    )
    def test_voltage_divider_vout_given_resistances(
        self, name, r2, r1, v_in, expected_v_out
    ):
...
class TestDigitalCountGivenVoltage:
    @pytest.mark.parametrize(
        "analog_in, v_max, expected_digital_count",
        [(-5, 10, -16383.5), (5, 10, 16383.5), (10, 10, 32767)],
    )
    def test_spot_check_digital_count_given_voltage(
        self, analog_in, v_max, expected_digital_count
    ):
...
class TestDigitalCountAndTemperatureFunctions:
    @pytest.mark.parametrize(
        "temperature, resistor, expected_digital_count", SPOT_CHECK_TEST_CASES
    )
    def test_spot_check_digital_count_given_temperature(
        self, temperature, resistor, expected_digital_count
    ):
        actual = module.digital_count_given_temperature(
            temperature, voltage_divider_resistor=resistor
        )
        assert pytest.approx(actual) == expected_digital_count

    @pytest.mark.parametrize(
        "expected_temperature, resistor, digital_count", SPOT_CHECK_TEST_CASES
    )
    def test_spot_check_temperature_given_digital_count(
        self, expected_temperature, resistor, digital_count
    ):
        actual = module.temperature_given_digital_count(
            digital_count, voltage_divider_resistor=resistor
        )
    ...
    def test_digital_count_given_temperature_round_trip(self):
        ...
        digital_counts = module.digital_count_given_temperature(
            temperatures, voltage_divider_resistor=resistor
        )
        round_trip_temperatures = module.temperature_given_digital_count(
            digital_counts, voltage_divider_resistor=resistor
        )
