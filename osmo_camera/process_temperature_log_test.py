...
class TestProcessTemperatureLog:
    def test_parses_raw_data_and_applies_calibration(self, mocker, tmp_path):
        ...
        expected_temperature_data = pd.DataFrame({
            'capture_timestamp': [datetime(2019, 4, 30, 16, 29, 5), datetime(2019, 4, 30, 16, 29, 11)],
            'digital_count': [20007, 19993],
            'voltage': [2.5003263, 2.5025763],
            'temperature_c': [sentinel.temperature, sentinel.temperature]
        })

        pd.testing.assert_frame_equal(actual, expected_temperature_data, check_less_precise=True)
