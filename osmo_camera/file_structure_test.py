...
class TestCreateOuputDirectory:
    def test_returns_output_directory_path(
        self, mock_path_exists, mock_path_join, mock_makedirs
    ):
    ...
    def test_creates_output_directory_if_doesnt_exist(
        self, mock_path_exists, mock_path_join, mock_makedirs
    ):
    ...
    def test_doesnt_create_output_directory_if_exists(
        self, mock_path_exists, mock_path_join, mock_makedirs
    ):
...
def test_get_files_with_extension(
    mock_listdir, mock_path_join, name, directory_contents
):
...
class TestAlteringFilepath:
    def test_append_suffix_to_filepath_before_extension(self):
        actual = module.append_suffix_to_filepath_before_extension(
            "/dir/dir/image.jpeg", "_i_am_a_filepath_suffix"
        )
