...
@pytest.mark.parametrize("name, test_rgb_image", [
    ...
    ('Within padded [-64, 64) DNR', np.array([
        [[-64, -64, -64], [-0.5, -0.5, -0.5]],
        [[0.5, 0.5, 0.5], [63.999999, 63.999999, 63.999999]]
    ])),
    ('Challenging high-precision fractions', np.array([
        [[1 / 3, 2**-25, 2**-29], [np.pi, np.e, -np.pi]]
    ])),
])
def test_rgb_image_saved_to_tiff_file_and_loaded_retains_data(tmp_path, name, test_rgb_image):
    absolute_tolerance = 1 / DNR_TO_TIFF_FACTOR  # quantization error when saving to and reading from an rgb image
    assert 1 / DNR_TO_TIFF_FACTOR == 2**-25  # Reminder to take another look at this test if the constant changes
    ...
    np.testing.assert_allclose(test_rgb_image, actual_tmp_tiff_as_rgb_image, atol=absolute_tolerance)
